...
LEGACY_STORE_DIRNAME = ".agent-skills"
...
def _warn_legacy_store(repo_root: Path) -> None:
    legacy = repo_root / LEGACY_STORE_DIRNAME
    new = repo_root / STORE_DIRNAME
    if legacy.exists() and not new.exists():
        print(
            f"Warning: found legacy store at {legacy} but no store at {new}.\n"
            "Run `python3 <SKILL_DIR>/scripts/self_learning.py migrate` to move your data.",
            file=sys.stderr,
        )
